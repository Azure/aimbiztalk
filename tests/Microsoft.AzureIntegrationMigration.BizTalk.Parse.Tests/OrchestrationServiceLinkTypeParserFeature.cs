...
{
    /// <summary>
    /// Tests for the <see cref="OrchestrationServiceLinkTypeParser"/> class.
    /// </summary>
    public class OrchestrationServiceLinkTypeParserFeature
    {
        ...
        public void ConstructWithNullModel(IBizTalkParser parser, ILogger logger, IApplicationModel model, MigrationContext context, Exception e)
        {
            ...
            "When constructing with null model"
                .x(() => e = Record.Exception(() => new OrchestrationServiceLinkTypeParser(model, context, logger)));
        ...
        public void ConstructWithSuccess(IBizTalkParser parser, ILogger logger, IApplicationModel model, MigrationContext context, Exception e)
        {
            ...
            "When constructing"
                .x(() => e = Record.Exception(() => parser = new OrchestrationServiceLinkTypeParser(model, context, logger)));
        ...
        [Scenario]
        [Trait(TestConstants.TraitCategory, TestConstants.CategoryUnitTest)]
        public void ParseIsSkippedIfModelIsMissing(OrchestrationServiceLinkTypeParser parser, ILogger logger, MigrationContext context, AzureIntegrationServicesModel model, Exception e)
        {
            "Given a model"
                .x(() =>
                {
            ...
            "And a parser"
                .x(() => parser = new OrchestrationServiceLinkTypeParser(model, context, logger));
        ...
        /// <summary>
        /// Scenario tests the happy path when a service link type is parsed.
        /// </summary>
        /// <param name="parser">The parser.</param>
        /// <param name="logger">The logger.</param>
        /// <param name="context">The migration context.</param>
        ...
        [Scenario]
        [Trait(TestConstants.TraitCategory, TestConstants.CategoryUnitTest)]
        public void ParsePortTypeWithSuccess(OrchestrationServiceLinkTypeParser parser, ILogger logger, MigrationContext context, AzureIntegrationServicesModel model, Exception e)
        {
            var orchestrationDefinitionName = "orchestrationDefinitionName";
            var orchestrationDefinitionKey = "orchestrationDefinitionKey";
            var serviceLinkTypeName = "serviceTypeName";
            var asmContainerKey = "asmContainerKey";

            "Given a source model with an orchestration and a service link type"
               .x(() =>
               {
                            ...
                            new Element
                            {
                                ...
                                Element1 = new Element[]
                                {
                                    new Element
                                    {
                                        ...
                                        {
                                            new ElementProperty { Name = "Name", Value = serviceLinkTypeName }
                                        }
                                    }
                                }
                            }
            ...
            "And the resources should be set."
                .x(() =>
                {
                    // Check the service link type resource has been created.
                    ...
                    var serviceLinkTypeResource = model.FindResourcesByType(ModelConstants.ResourceServiceLinkType).SingleOrDefault();
                    serviceLinkTypeResource.Should().NotBeNull();
                    var serviceLinkType = ((ParsedBizTalkApplicationGroup)model.MigrationSource.MigrationSourceModel).Applications[0].Application.Orchestrations[0].FindServiceLinkTypes().Single();

                    // Validate the service link type resource.
                    serviceLinkTypeResource.Should().NotBeNull();
                    serviceLinkTypeResource.Key.Should().Be(string.Concat(moduleResource.Key, ":", serviceLinkTypeName));
                    serviceLinkTypeResource.Name.Should().Be(serviceLinkTypeName);
                    serviceLinkTypeResource.Type.Should().Be(ModelConstants.ResourceServiceLinkType);

                    serviceLinkType.Resource.Should().Be(serviceLinkTypeResource); // The pointer to the resource should be set.
                    serviceLinkTypeResource.ParentRefId.Should().Be(moduleResource.RefId); // The parent ref ID should be set.
                    serviceLinkTypeResource.SourceObject.Should().Be(serviceLinkType); // The resource should have a pointer to the source object. 
                });
        }
        ...
        [Scenario]
        [Trait(TestConstants.TraitCategory, TestConstants.CategoryUnitTest)]
        public void ParsePortTypeWithMissingModule(OrchestrationServiceLinkTypeParser parser, ILogger logger, MigrationContext context, AzureIntegrationServicesModel model, Exception e)
        {
            var orchestrationDefinitionName = "orchestrationDefinitionName";
            var orchestrationDefinitionKey = "orchestrationDefinitionKey";
            var asmContainerKey = "asmContainerKey";
